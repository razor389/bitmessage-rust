...
mod tests {
    ...
    fn test_encryption_decryption() {
        ...
        let (ciphertext, nonce) = encryption_a.encrypt_message(&encryption_b.our_public_key, message);

        let plaintext = encryption_b.decrypt_message(&encryption_a.our_public_key, &nonce, &ciphertext);
    ...
    fn test_signing_verification() {
        ...
        assert!(Authentication::verify_message_with_key(message, &signature, &sender_public_key));

        // Verify fails with an incorrect public key
        let wrong_sender_public_key = auth_b.verifying_key();
        assert!(!Authentication::verify_message_with_key(message, &signature, &wrong_sender_public_key));
    }
